...
void pnet_model::ReadInput(site_struct* site, veg_struct* veg, share_struct* share, char* inputname)
{
	...
	fscanf(f_input_load, "%lf", &share->param); fgets(note, 200, f_input_load);
...
void pnet_model::WriteoutDay(site_struct* site, veg_struct* veg, clim_struct* clim, share_struct* share, int rstep, FILE*& fileoutM)
{
	...
	if (iheader == 1)
	{
		...
		fprintf(fileoutM, " year,   doy,  temp,   GrowthDD,  "
			...
			"airT, NetNMinYr, NetNitrYr,NRatioNit,NDrain,UptakeEff,FoliarN, "
			"PlantN,   WoodN,    RootN,  SnowNH4, SnowNO3,"
			"NH4, NO3,Freeze,FreezeDoy" //Linghui 20200511
			"\n");
	...
	fprintf(fileoutM, "%10.2f,%10.5f,%10.5f,%10.5f,%10.5f,%10.5f,",
		...
		share->NRatioNit,
		share->NDrain,
		share->UptakeEff);//share->WFPSshare->DayLength

	fprintf(fileoutM, "%7.3f,",
		share->FolNCon);//Foliar N

	fprintf(fileoutM, "%8.2f, %8.2f, %8.2f,",
		share->PlantN,
		share->WoodMassN,
		share->RootMassN);
	fprintf(fileoutM, "%10.5f, %10.5f,",
		//			share->CanopyDO3,
		share->SnowNH4,
		share->SnowNO3);
	//		share->CanopyGrossPsn);

	fprintf(fileoutM, "%10.5f, %10.5f,%8.2f, % 8.2f,",
		share->NH4,	//Linghui 20200511
		share->NO3,
		share->EnvMaxFol,
		share->NdepTot_prev); //Linghui 20200511
...
int pnet_model::Disturbance(site_struct* site, veg_struct* veg, share_struct* share, int scenario)
{
	...
	if (scenario == 0) // pre-define 15 disturbance events
	{
		...
		site->distRootloss = (double*)malloc((site->distyrs + 1) * sizeof(double));
	...
	if (scenario == 1) //HB w6
	{
		...
		site->folregen[4] = 100;
		site->distRootloss[4] = 0;

		site->distyear[5] = 1990; //soil freeze, convert part of bud C and N to dead wood
		site->distintensity[5] = 0.0;
		site->distremove[5] = 0;
		site->distsoilloss[5] = 0.0;
		site->folregen[5] = 100;
		site->distRootloss[5] = 0.5;

		site->distyear[6] = 1999; //ice storm event at start of 1998, convert part of bud C and N to dead wood
		site->distintensity[6] = 0;
		site->distremove[6] = 0;
		site->distsoilloss[6] = 0.0;
		site->folregen[6] = 100;
		site->distRootloss[6] = 0.3;

		site->distyear[7] = 2014; //soil freeze, convert part of bud C and N to dead wood
		site->distintensity[7] = 0;
		site->distremove[7] = 0;
		site->distsoilloss[7] = 0.0;
		site->folregen[7] = 100;
		site->distRootloss[7] = 0.3;
